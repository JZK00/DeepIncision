...
def do_coco_evaluation(
    ...
    for iou_type in iou_types:
        with tempfile.NamedTemporaryFile() as f:
            ...
            if output_folder:
                ...
                file_path = output_folder+ "_" + iou_type + ".json"
            if dataset.coco:
    ...
    if output_folder:
        ...
        torch.save(results, output_folder + "bbox.pth")
...
def evaluate_predictions_on_coco(
    ...
    if iou_type == 'bbox':
        ...
        # # IoU=0.5 对应的索引（通常是0）
        # iou_thr_list = coco_eval.params.iouThrs
        # iou_index = list(iou_thr_list).index(0.5)

        # print("\nPer-category AUC (PR Curve Area @IoU=0.50 only):")
        # precisions = coco_eval.eval['precision']  # [T, R, K, A, M]
        # cat_ids = coco_gt.getCatIds()
        # assert len(cat_ids) == precisions.shape[2]

        # for idx, catId in enumerate(cat_ids):
        #     pr = precisions[iou_index, :, idx, 0, -1]  # shape [R], IoU=0.5 only
        #     valid = pr > -1
        #     auc = np.mean(pr[valid]) if np.any(valid) else float("nan")
...
def summarize_per_category(coco_eval, csv_output=None):
    '''
    Only output per-category AP@0.5 and AR@0.5
    '''

    def _summarize_ap_050():
        p = coco_eval.params
        result_str = ' Average Precision  (AP) @[ IoU=0.50      | area=   all | maxDets=100 ], '

        aind = [i for i, aRng in enumerate(p.areaRngLbl) if aRng == 'all']
        mind = [i for i, mDet in enumerate(p.maxDets) if mDet == 100]
        t = np.where(p.iouThrs == 0.5)[0]

        s = coco_eval.eval['precision'][t, :, :, aind, mind]  # [1, R, K, 1, 1]
        # import pdb;pdb.set_trace()
        s = np.squeeze(s)
        ...
        for i in range(num_classes):
            class_ap = np.mean(s[:, i][s[:, i] > -1])
            result_str += '{:.6f}, '.format(class_ap)
            avg_ap += class_ap
        result_str += '{:.6f} \n'.format(avg_ap / num_classes)
        return result_str

    def _summarize_ar_050():
        p = coco_eval.params
        result_str = ' Average Recall     (AR) @[ IoU=0.50      | area=   all | maxDets=100 ], '

        aind = [i for i, aRng in enumerate(p.areaRngLbl) if aRng == 'all']
        mind = [i for i, mDet in enumerate(p.maxDets) if mDet == 100]
        t = np.where(p.iouThrs == 0.5)[0]

        s = coco_eval.eval['recall'][t, :, aind, mind]  # [1, K, 1, 1]
        s = np.squeeze(s)
        
        num_classes = len(p.catIds)
        avg_ar = 0.0
        for i in range(num_classes):
            class_ar = s[i] if s[i] > -1 else 0
            result_str += '{:.6f}, '.format(class_ar)
            avg_ar += class_ar
        result_str += '{:.6f} \n'.format(avg_ar / num_classes)
        return result_str
    ...
    results = [title_str]
    results.append(_summarize_ap_050())
    results.append(_summarize_ar_050())
