...
class VLDyHeadModule(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.ADD_LINEAR_LAYER:
            tunable_linears = []
            for i in range(self.cfg.MODEL.DYHEAD.FUSE_CONFIG.TASK):
                
                tunable_linear = torch.nn.Linear(self.lang_dim, 1000, bias=False)
                tunable_linear.weight.data.fill_(0.0)
                tunable_linears.append(tunable_linear)
            # self.mask = torch.zeros((1000, self.lang_dim))
            self.tunable_linears = nn.ModuleList(tunable_linears)
            # self.mask_linear.weight.data.fill_(0.0)
            # self.zero_convs=nn.ModuleList([
            #     self.make_zero_conv() for i in range(5)
            # ])
    ...
    def make_zero_conv(self):
        return zero_module(nn.Conv2d(self.joint_embedding_size, self.joint_embedding_size, 1, padding=0))
    ...
    def forward(self, images, features, targets=None,
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.ADD_LINEAR_LAYER:
            index = self.cfg.MODEL.DYHEAD.FUSE_CONFIG.TASK - 1
            
            if index > 0:
                for i in range(index):
                    self.tunable_linears[index].weight.data += self.tunable_linears[0].weight.data


            # self.tunable_linear.weight *= self.mask
            # import pdb;pdb.set_trace()
            embedding[:, :,(index-1)*50 : index* 50] += self.tunable_linears[index].weight[:embedding.size(1),  (index-1)*50 : index* 50].unsqueeze(0) 
            language_dict_features['embedded'] = embedding
            language_dict_features['hidden'][:,:, (index-1)*50 : index* 50] += self.tunable_linears[index].weight[:embedding.size(1),  (index-1)*50 : index* 50].unsqueeze(0) 
            # import pdb;pdb.set_trace()
            # features = list(features)
            # for i in range(len(features)):
            #     x = self.zero_convs[i](features[i])
            #     features[i] += x
