...
def main():
    ...
    cfg.merge_from_file(args.prompt_file)
    ...
    logger.info("Loaded prompt configuration file {}".format(args.prompt_file))
    with open(args.prompt_file, "r") as cf:
        config_str = "\n" + cf.read()
            ...
            if log_dir:
                for idx, dataset_name in enumerate(dataset_names):
                    output_folder = os.path.join(log_dir, "inference", dataset_name,
                                                 args.exp_name + datetime.now().strftime("%Y_%m_%d-%I_%M_%S"))
                    mkdir(output_folder)
